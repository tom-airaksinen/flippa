...
let statsPrioPopOpen = false;    // PRIO-dropdownen i statistikens Leitner-rad öppen?
let statsPrioOutsideBound = false; // dokument-lyssnare för utsidesklick bunden en gång
...
function setTab(tab) {
  ...
  if (tab === "stats") {
    ...
    statsPrioPopOpen = false; // PRIO-dropdownen börjar stängd
    renderStats();
  }
  updateTabbar();
}
...
const PROFILE_COLORS = { tom: "#5b8cff", hedvig: "#ff3d8f", wille: "#5bbf72", karin: "#c77dff", martin: "#ff8a3d", maria: "#2ec4b6", harry: "#e5484d", guest: "#9aa3b2" };
...
function openAiPrefModal() {
  ...
  const VAL = [
    ...
    { v: "copy",   ic: () => scaleIcon(COPY_ICON_SVG, 18.0), t: "Kopiera frågan" },
    { v: "",       ic: () => scaleIcon(AI_STARS_SVG, 19.8),  t: "Fråga varje gång" },
  ];
  const m = openModal(`
    <h3>AI-hjälp</h3>
...
function renderStats() {
  ...
  body.innerHTML = `
    ...
    <div class="st-sec st-leitner-head">
      <span>LEITNER · ${ltTotal} kort</span>
      <div class="st-prio-wrap">
        <button type="button" class="st-prio-pill" id="st-prio-pill" aria-haspopup="true">PRIO<span class="limit-dots">${[1, 2, 3].map((l) => `<i class="p${l}${stPrioSel.includes(l) ? "" : " off"}"></i>`).join("")}</span></button>
        <div class="st-prio-pop${statsPrioPopOpen ? "" : " hidden"}" id="st-prio-pop">${[1, 2, 3].map((l) => `<button type="button" data-lvl="${l}" class="${stPrioSel.includes(l) ? "on" : ""}"><span class="prio-dot p${l}"></span>Prio ${l}</button>`).join("")}</div>
      </div>
    </div>
  ...
  // Diskret PRIO-väljare på Leitner-raden: pill (med prickar) → liten dropdown med toggles.
  const stPill = body.querySelector("#st-prio-pill");
  const stPop = body.querySelector("#st-prio-pop");
  if (stPill && stPop) {
    stPill.onclick = () => { statsPrioPopOpen = !statsPrioPopOpen; stPop.classList.toggle("hidden", !statsPrioPopOpen); };
    stPop.addEventListener("click", (e) => {
      ...
      statsPrioPopOpen = true; // håll dropdownen öppen medan man togglar
      renderStats();
    });
  }
  // Stäng dropdownen vid klick utanför (bind dokument-lyssnaren en gång).
  if (!statsPrioOutsideBound) {
    statsPrioOutsideBound = true;
    document.addEventListener("pointerdown", (e) => {
      if (!statsPrioPopOpen) return;
      if (e.target.closest("#st-prio-pill") || e.target.closest("#st-prio-pop")) return;
      statsPrioPopOpen = false;
      const p = document.getElementById("st-prio-pop"); if (p) p.classList.add("hidden");
    });
  }
...
// Ikoner ritar olika mycket inuti sin 24-ruta: ChatGPT-flätan fyller den helt (24,0
// enheter) medan kopiera-ikonen bara är 18,0. I samma lista blir det spretigt trots
// att boxarna är lika stora. scaleIcon() skalar innehållet kring mitten så att den
// synliga ytan – inte rutan – blir lika stor. Måttet är uppmätt med getBBox().
const ICON_TARGET = 20;   // önskad bläckyta i viewBox-enheter (genomsnitt av bredd/höjd)
// naturlig = uppmätt genomsnitt av bläckytans bredd och höjd (getBBox).
function scaleIcon(svg, naturlig) {
  const k = (ICON_TARGET / naturlig).toFixed(3);
  return svg.replace(/(<svg[^>]*>)([\s\S]*)(<\/svg>)/,
    (_, start, inner, slut) =>
      `${start}<g transform="translate(12 12) scale(${k}) translate(-12 -12)">${inner}</g>${slut}`);
}
...
async function editCurrentCard() {
  ...
  const res = await askWord(c.front, c.back, c.hint, { allowDelete: true, explore: !!subjectLang(subj), lessons: subj.lessons, lessonId: lid, prio: c.prio, forms: !!subj.forms, form: c.form, tr: c.tr });
  if (!res) return;
  if (res._delete) {
  ...
  if (res.lessonId && res.lessonId !== lid) {
    moveCard(currentSubject.id, lid, res.lessonId, c.id,
             { front: res.front, back: res.back, hint: res.hint, form: res.form, tr: res.tr, prio: res.prio });
  } else {
...
function askWord(front, back, hint, opts = {}) {
  ...
  return new Promise((resolve) => {
    ...
    const open = (f, b, h, fo, tl) => {
      ...
      // Uttalsfältet följer samma logik som kortet: bara språk som använder
      // translitterering (i dag persiska) har det, och det står direkt under ordet
      // eftersom det hör till det utländska ordet – precis som böjningen.
      const trBlock = subjectUsesTranslit() ? `
      <label>Uttal med latinska bokstäver (valfritt)</label>
      <input type="text" id="m-tr" value="${esc(tl || "")}" placeholder="t.ex. salâm" autocomplete="off" autocapitalize="none" spellcheck="false" />` : "";
      ...
      <input type="text" id="m-front" value="${esc(f)}" dir="auto" autocomplete="off" autocapitalize="none" autocorrect="off" spellcheck="false" />${trBlock}${formBlock}
      ...
      <div class="hint-lbl">
        ...
        <div class="ai-pop-wrap">
          ...
          <div class="ai-pop hidden" id="m-ai-pop">
            ...
            <button type="button" class="ai-pop-item" id="m-ai-copy">${scaleIcon(COPY_ICON_SVG, 18.0)}Kopiera frågan</button>
      ...
      m.querySelector("#m-ok").onclick = () => {
        ...
        closeModal();
        resolve(v.f && v.b ? { front: v.f, back: v.b, hint: v.h, form: v.fo, tr: v.tl, lessonId: chosenLid, prio: curPrio } : null);
      };
    };
    open(front, back, hint, form, tr);
  });
}
...
function updateCard(sid, lid, cid, fields) {
  ...
  const value = {
    ...
    tr: fields.tr || null,   // translitterering, tomt fält tar bort den
...
// fields: { front, back, hint, form, tr, prio } – se kommentaren vid updateCard.
function moveCard(sid, fromLid, toLid, cid, fields) {
  ...
  if (fields.tr) card.tr = fields.tr;
...
async function editWord(cid) {
  ...
  if (!c) return;
  const subj = freshSubject();
  const res = await askWord(c.front, c.back, c.hint, { allowDelete: true, explore: !!subjectLang(subj), lessons: subj.lessons, lessonId: lesson.id, prio: c.prio, forms: !!subj.forms, form: c.form, tr: c.tr });
  if (!res) return;
