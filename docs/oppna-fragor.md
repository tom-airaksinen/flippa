...
- [ ] Räknas jag/tester in flera gånger i GoatCounter? → **Analys (2026-07-07):** Inga fantomanvändare, men egen trafik inflateras: (a) **varje app-laddning = 1 sidvisning**, och PWA:n auto-omladdar vid ny version (`skipWaiting`+reload) → en deploy-dag ger flera visningar per öppning; (b) GoatCounter härleder "besökare" ur en **dygnsroterande** hash av IP+UA → man räknas som ny besökare varje dag (en daglig användare ≈ 30 besökare/mån); (c) ett pass avfyrar många events (uttala/pass-klart/…) som visas separat. **Automattester räknas EJ** – verify-harnessen blockerar goatcounter.com. Self-exclude ej byggt: `#toggle-goatcounter` sätter `localStorage.skipgc` men når inte den installerade hemskärms-PWA:n (egen storage, fast start_url); IP-ignore opålitligt på mobil. **Öppet val:** bygga en diskret "räkna inte mig"-toggle (sätter skipgc + tidig return i vår egen `track()`, som idag kringgår filtret för nav-events)?
...
- [ ] Nivå 1 – **generisk** daglig påminnelse ("Dags att plugga i Flippa!")? → Kräver INTE per-uid-data; klient + liten schemalagd sändare. Rimligt tidigt vinst-case inför fler testare. Uppskattning: ~1–2 dagars jobb, mest friktion i VAPID-setup + att resa cron-sändaren. **Öppet:** var bor sändaren – Cloud Function (Blaze) eller Cloudflare Worker?
...
- [ ] Var bor sändaren för nivå 1? → **Rek. (2026-07-07):** GitHub Actions schemalagt workflow (gratis, inget nytt konto, ingen Blaze) läser subscriptions ur RTDB och skickar via `web-push`+VAPID. Alt: Cloudflare Worker (gratis, extra plattform) eller Cloud Function+Scheduler (firmare timing men kräver Blaze). Ingen egen dator behövs – cron kör i molnet. Jag kan sätta GitHub-hemligheter via `gh` (admin-scope finns); du hämtar service-account-nyckel + klistrar RTDB-regler i konsolen (~5–10 min engångs).
- [ ] Inställningsvy/profil – mockup med states: [`mockups/installningar.html`](../mockups/installningar.html). Visar notis-kortet i webb-vs-PWA × ej-frågad/tillåten/nekad, samt användarväljaren i first-launch (ingen profil vald) vs byt-användare (lösenordsdialog vid låst profil + plats för framtida Logga ut). **Öppet:** bygga tunn inställningsvy som ställning nu (flytta in mål & backup; auto-uppläsning stannar i Flippa-vyn – måste vara lättåtkomlig, t.ex. på bussen) och lägga push som första riktiga toggle, eller köra push fristående först?
